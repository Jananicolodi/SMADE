...
if($conexao){
    $query = "insert into aluno(nome,ano,data_nascimento,senha,idEscola,idTurma,idResponsavel) values(?,?,?,?,?,?,?)";
    $query_tratada = mysqli_prepare($conexao,$query);    
    if($query_tratada){
        mysqli_stmt_bind_param($query_tratada,"ssssiii",$nome,$ano,$dataNascimento,$senha,$idEscola,$idTurma,$idResponsavel);
